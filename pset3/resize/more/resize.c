...
int main(int argc, char *argv[])
{
    ...
    // sscanf returns a number of items parsed
    if (sscanf(argv[1], "%f", &factor) != 1 || factor < 0.0 || factor > 100.0)
    {
        fprintf(stderr, "factor must be a floating-point value in [0.0, 100.0]\n");
        return 1;
    }
